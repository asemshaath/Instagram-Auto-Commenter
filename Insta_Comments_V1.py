import requests,sys,re,json,secrets
...
''')
    print(lo+m('                                       CopyRight: Asem Shaath         \n'))
...
different_headers = [
        # Here put cookies for instagram accounts to give likes on the comments using them.  example of the cookies:
        # {'cookie': 'mid=******************; ig_did=*******-****-4***-****-********; csrftoken=********************; datr=*************; shbid=*****;  ds_user_id=*******; sessionid=********%*************%***; shbts=***********.******; rur=RVA ig_cb=*; '}
]
...
def logins():
    for username in accounts.keys():
        ...
        headers_login = {
            ...
            'cookie': cokie,#'csrftoken=afbTnjYMox2iZZ2FwMGlROfIQeIQLFpF; mid=YMDJAQALAAE_L1bTszWakhT3iV4a; ig_did=374D1E6F-6771-4858-A904-94D3F0B3290D',
    ...
    def likes(data):
        ...
        for head in different_headers:
            like_headers = {
                ...
                'x-csrftoken': re.findall(r'csrftoken\=\S{,}',head["cookie"])[0][10:-1],
                'x-ig-app-id': '936619743392459',
                'x-ig-www-claim': 'hmac.AR3EEYh8LsHjPpwwzN58KgFJAV7AoI8G-2ZDAkaoChtjHJxZ',
            ...
            liky = True
            try:
                like = requests.post(f'https://www.instagram.com/web/comments/like/{data["id"]}/',headers=like_headers,timeout=5)
            except:
                liky = False
            if liky and '"status":"ok"' in like.text:
                l += 1
                print(ha+' Like number '+c(str(l))+' done!')
    try:
...
if len(sessions) > 0:
    ...
    sleep(5)
    #  appending logged accounts to liking
    for a in cook:
        cs = {'cookie':f'ig_did=D37E2F4F-6193-4E76-B7C6-6FDBE4A0C230; mid=X_3LtAALAAFAdQMURlFUf_U68Q6H; ig_nrcb=1; csrftoken={a["csrftoken"]}; shbid=11548; shbts=1615465726.6437705; rur=ASH,ds_user_id={a["ds_user_id"]}; sessionid='+a["sessionid"]}
        different_headers.append(cs)
